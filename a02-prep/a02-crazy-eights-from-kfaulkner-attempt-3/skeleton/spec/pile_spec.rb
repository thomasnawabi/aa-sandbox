...
describe Pile do
  ...
  describe "#valid_play?" do
    it "approves playing a card of the same suit" do
      expect(pile.valid_play?(Card.new(:clubs, :three))).to be_true
    end

    it "aproves playing a card of the same rank" do
      expect(pile.valid_play?(Card.new(:diamonds, :deuce))).to be_true
    end

    it "approves any eight" do
      expect(pile.valid_play?(Card.new(:diamonds, :eight))).to be_true
    end
  ...
  describe "#play_eight" do
    ...
    it "affects what cards can be next played" do
      ...
      expect(pile.valid_play?(Card.new(:hearts, :four))).to be_true
    end
    ...
    it "doesn't affect suits of subsequent plays" do
      ...
      expect(pile.current_suit).to eq(:clubs)
      expect(pile.valid_play?(Card.new(:clubs, :five))).to be_true
    end
  end
end
